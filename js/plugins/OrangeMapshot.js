 ...
 *=============================================================================*/
/*:
 * @plugindesc (v1.7)[v1.0]  系统 - 截取地图工具
 ...
 * @on 使用
 * @off 不使用
 * @desc true - 使用，false - 不使用，如果不使用地图名，存储的文件将会是数字。
 * @default true
 *
 * @param 存储图层
 * @type select
 ...
 * @on 绘制
 * @off 不绘制
 * @desc true - 绘制，false - 不绘制，地图中的阴影会被绘制进去。
 * @default true
 *
 * @param 是否绘制事件
 * @type boolean
 * @on 绘制
 * @off 不绘制
 * @desc true - 绘制，false - 不绘制，地图中的NPC会被绘制进去。
 * @default true
 *
 * @param 截取按键
 * @type select
 ...
 * @min 0
 * @max 100
 * @desc 如果选择了jpeg或者webp，你可以设置图片的0-100的质量。100为质量最高。（png的质量就是100，不需要设置）
 * @default 70
 *
 * @param 目标文件夹
 * @desc 存储的目标文件夹名路径，文件夹会自动创建。"./"表示当前项目的路径。
 * @default ./地图截取
 *
 * @help
 * =============================================================================
 ...
 * =============================================================================
 * 在游戏中，按指定的按键，可以截取游戏整个地图，并存储到目标路径中。
 * （rmmv原图片存储功能会被缩小一半，该插件是1:1存储图片。）
 * 截取成功时，会自动弹出设置的文件夹的窗口。
 *
 * -----------------------------------------------------------------------------
 * ----设定注意事项
 * 1.插件的作用域：地图界面。
...
(function($) {
  ...
  var Parameters = PluginManager.parameters('OrangeMapshot');
  if (Parameters.length === 0) {
    throw new Error("Couldn't find OrangeMapshot parameters.");
  }
  
  $.Param = {};
  $.Param.useMapName = Parameters['是否使用地图名'] !== "false";
  $.Param.drawAutoShadows = Parameters['是否绘制阴影'] !== "false";
  $.Param.drawEvents = Parameters['是否绘制事件'] !== "false";
  $.Param.layerType = Number(Parameters['存储图层'] || 0);
  $.Param.imageType = Parameters['图片类型'] || 'png';
  $.Param.imagePath = Parameters['目标文件夹'] || './Mapshots';
  $.Param.imageQuality = Number(Parameters['图片质量'] || 70);

  $.Param.keyCode = Number(Parameters['截取按键'] || 118);
